...
public class PenghitungBeratBadan extends Application {
    ...
    private void simpan() {
        // Mendapatkan nilai-nilai BMI, kategori, dan saran dari label-label
    ...
    private void Showdata() {
        try (BufferedReader reader = new BufferedReader(new FileReader("hasil_bmi.txt"))) {
            String hasilBMI = reader.readLine();
            String kategori = reader.readLine();
            String saran = reader.readLine();

            // Menetapkan nilai-nilai dari file ke label-label
            labelHasil.setText(hasilBMI);
            labelKategori.setText(kategori);
            labelSaran.setText(saran);
